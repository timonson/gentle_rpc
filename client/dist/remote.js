...
function send(resource, fetchInit) {
    return fetch(resource instanceof URL ? resource.href : resource, fetchInit).then((res)=>res.status === 204 || res.headers.get("content-length") === "0" ? undefined : res.text().then((text)=>text ? JSON.parse(text) : undefined).catch((err)=>Promise.reject(new BadServerDataError(null, "The received data is invalid JSON."))));
}
function processBatchArray(rpcResponseBatch, isNotification) {
    return rpcResponseBatch.map((rpcResponse)=>validateResponse(rpcResponse, isNotification).result);
}
...
class Remote {
    ...
    call(method, params, { isNotification , jwt  } = {}) {
        ...
        }).then((rpcResponse)=>rpcResponse === undefined && isNotification ? undefined : validateResponse(rpcResponse, isNotification).result);
    }
}
        ...
        this.payloadData = new Promise((resolve, reject)=>{
            ...
            socket.onmessage = async (event)=>{
                ...
                try {
                    ...
                    reject(new BadServerDataError(null, "The received data is invalid JSON."));
                }
            };
...
function createRemote(resourceOrSocket, options) {
    return resourceOrSocket instanceof WebSocket ? listen(resourceOrSocket).then((socket)=>new Remote1(socket)) : new Remote(resourceOrSocket, options);
}
