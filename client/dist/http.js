...
function createRequestBatch(batchObj) {
  var isNotification = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  return Array.isArray(batchObj) ? batchObj.map(function (el) {
  ...
  }).flat(2) : Object.entries(batchObj).map(function (_ref4) {
...
var BadServerDataError = /*#__PURE__*/function (_Error) {
  ...
  return _createClass(BadServerDataError);
}( /*#__PURE__*/_wrapNativeSuper(Error));

function validateRpcBasis(data) {
  return (data === null || data === void 0 ? void 0 : data.jsonrpc) === "2.0" && (typeof data.id === "number" || typeof data.id === "string" || data.id === null);
}
...
function validateRpcFailure(data) {
  ...
  return typeof (data === null || data === void 0 ? void 0 : (_data$error = data.error) === null || _data$error === void 0 ? void 0 : _data$error.code) === "number" && typeof data.error.message === "string";
}

function validateResponse(data, isNotification) {
  if (isNotification && data !== undefined) {
    throw new BadServerDataError(null, "The server's response to the notification contains unexpected data.");
  }

  if (validateRpcBasis(data)) {
    if (validateRpcSuccess(data)) return data;else if (validateRpcFailure(data)) {
      throw new BadServerDataError(data.id, data.error.message, data.error.code, data.error.data);
    }
  }

  throw new BadServerDataError(null, "The received data is no valid JSON-RPC 2.0 Response object.");
}

function send(resource, fetchInit) {
  return fetch(resource instanceof URL ? resource.href : resource, fetchInit).then(function (res) {
    return res.status === 204 || res.headers.get("content-length") === "0" ? undefined : res.text().then(function (text) {
      return text ? JSON.parse(text) : undefined;
    })["catch"](function (err) {
      return Promise.reject(new BadServerDataError(null, "The received data is invalid JSON."));
    });
  });
}
...
function processBatchObject(rpcResponseBatch, isNotification) {
  return rpcResponseBatch.reduce(function (acc, rpcResponse) {
    ...
    if (rpcSuccess.id !== null) {
      ...
      throw new BadServerDataError(null, "Type 'null' cannot be used as an index type.");
    }
  }, {});
}

var Remote = /*#__PURE__*/function () {
  function Remote(resource) {
    ...
    var headers = options.headers === undefined ? new Headers() : options.headers instanceof Headers ? options.headers : new Headers(options.headers);
  ...
  _createClass(Remote, [{
    ...
    value: function batch(batchObj) {
      ...
      return send(this.resource, _objectSpread(_objectSpread({}, this.fetchInit), {}, {
        headers: jwt ? new Headers([].concat(_toConsumableArray(this.fetchInit.headers.entries()), [["Authorization", "Bearer ".concat(jwt)]])) : this.fetchInit.headers,
        ...
        if (rpcResponseBatch === undefined && isNotification) {
          ...
          return Array.isArray(batchObj) ? processBatchArray(rpcResponseBatch, isNotification) : processBatchObject(rpcResponseBatch, isNotification);
        } else {
          throw new BadServerDataError(null, "The server returned an invalid batch response.");
        }
      });
    }
  }, {
    ...
    value: function call(method, params) {
      ...
      return send(this.resource, _objectSpread(_objectSpread({}, this.fetchInit), {}, {
        ...
        return rpcResponse === undefined && isNotification ? undefined : validateResponse(rpcResponse, isNotification).result;
      });
    }
  }]);
